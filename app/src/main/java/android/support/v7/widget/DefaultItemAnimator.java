...
public class DefaultItemAnimator extends RecyclerView.ItemAnimator
{
  ...
  private void animateChangeImpl(final ChangeInfo paramChangeInfo)
  {
    ...
    label171: for (Object localObject2 = ((RecyclerView.ViewHolder)localObject2).itemView; ; localObject2 = null)
    {
      if (localObject1 != null)
      {
        localObject1 = ViewCompat.animate((View)localObject1).setDuration(getChangeDuration());
        mChangeAnimations.add(paramChangeInfo.oldHolder);
        ...
        ((ViewPropertyAnimatorCompat)localObject1).alpha(0.0F).setListener(new VpaListenerAdapter(paramChangeInfo)
        {
          public void onAnimationEnd(View paramAnonymousView)
          {
            ...
            dispatchChangeFinished(paramChangeInfo.oldHolder, true);
            mChangeAnimations.remove(paramChangeInfo.oldHolder);
            DefaultItemAnimator.this.dispatchFinishedWhenDone();
          }

          public void onAnimationStart(View paramAnonymousView)
          {
            dispatchChangeStarting(paramChangeInfo.oldHolder, true);
          }
        }).start();
      }
      if (localObject2 != null)
      {
        ...
        mChangeAnimations.add(paramChangeInfo.newHolder);
        ((ViewPropertyAnimatorCompat)localObject1).translationX(0.0F).translationY(0.0F).setDuration(getChangeDuration()).alpha(1.0F).setListener(new VpaListenerAdapter(paramChangeInfo)
        {
          public void onAnimationEnd(View paramAnonymousView)
          {
            ...
            dispatchChangeFinished(paramChangeInfo.newHolder, false);
            mChangeAnimations.remove(paramChangeInfo.newHolder);
            DefaultItemAnimator.this.dispatchFinishedWhenDone();
          }

          public void onAnimationStart(View paramAnonymousView)
          {
            dispatchChangeStarting(paramChangeInfo.newHolder, false);
          }
        }).start();
      }
      return;
  ...
  public void endAnimation(RecyclerView.ViewHolder paramViewHolder)
  {
    ...
    while (i >= 0)
    {
      if (((MoveInfo)mPendingMoves.get(i)).holder == paramViewHolder)
      {
  ...
  public void runPendingAnimations()
  {
    ...
    if (!mPendingRemovals.isEmpty())
    {
      ...
      if (mPendingChanges.isEmpty())
        break label77;
      k = 1;
      label36: if (mPendingAdditions.isEmpty())
        break label82;
    }
    ...
    label88: Object localObject1 = mPendingRemovals.iterator();
