...
abstract class AbsSpinnerCompat extends AdapterViewCompat<SpinnerAdapter>
{
  ...
  protected void onMeasure(int paramInt1, int paramInt2)
  {
    ...
    if (i > mSelectionLeftPadding)
    {
      ...
      if (m <= mSelectionTopPadding)
        break label474;
      i = m;
      label67: ((Rect)localObject).top = i;
      localObject = mSpinnerPadding;
      if (k <= mSelectionRightPadding)
        break label482;
      i = k;
      label91: ((Rect)localObject).right = i;
      localObject = mSpinnerPadding;
      if (j <= mSelectionBottomPadding)
        break label490;
    }
    label474: label482: label490: for (i = j; ; i = mSelectionBottomPadding)
    {
  ...
  public void onRestoreInstanceState(Parcelable paramParcelable)
  {
    ...
    if (paramParcelable.selectedId >= 0L)
    {
      mDataChanged = true;
      mNeedSync = true;
      mSyncRowId = paramParcelable.selectedId;
      mSyncPosition = paramParcelable.position;
      mSyncMode = 0;
      requestLayout();
    }
  }
  ...
  void resetList()
  {
    ...
    mOldSelectedRowId = -9223372036854775808L;
