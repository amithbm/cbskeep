  ...
  private void executePendingBroadcasts()
  {
    while (true)
    {
      ...
      synchronized (mReceivers)
      {
        ...
        if (j < ???.receivers.size())
        {
          ((ReceiverRecord)???.receivers.get(j)).receiver.onReceive(mAppContext, ???.intent);
          j += 1;
        }
      }
      i += 1;
  ...
  public boolean sendBroadcast(Intent paramIntent)
  {
    ...
    if (i != 0)
      ...
      default:
        ...
        label392: Log.v("LocalBroadcastManager", "  Filter did not match: " + (String)localObject1);
        localObject1 = localObject2;
        break;
      case -3:
      case -4:
