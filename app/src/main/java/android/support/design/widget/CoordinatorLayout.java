  ...
  implements NestedScrollingParent
{
  ...
  private void layoutChild(View paramView, int paramInt)
  {
    ...
    {
      localRect1.left += mLastInsets.getSystemWindowInsetLeft();
      localRect1.top += mLastInsets.getSystemWindowInsetTop();
      localRect1.right -= mLastInsets.getSystemWindowInsetRight();
      localRect1.bottom -= mLastInsets.getSystemWindowInsetBottom();
    }
  ...
  void dispatchOnDependentViewChanged(boolean paramBoolean)
  {
    ...
    while (i < m)
    {
      ...
      else
      {
        ...
        j = i + 1;
        label155: Behavior localBehavior;
        if (j < m)
        {
        ...
        while (true)
        {
          ...
          break label155;
          break;
          label235: boolean bool = localBehavior.onDependentViewChanged(this, (View)localObject1, localView);
          if (paramBoolean)
  ...
  protected boolean drawChild(Canvas paramCanvas, View paramView, long paramLong)
  {
    ...
    if ((localLayoutParams.mBehavior != null) && (localLayoutParams.mBehavior.getScrimOpacity(this, paramView) > 0.0F))
    {
      ...
      mScrimPaint.setColor(localLayoutParams.mBehavior.getScrimColor(this, paramView));
  ...
  public void onDraw(Canvas paramCanvas)
  {
    ...
    if ((mDrawStatusBarBackground) && (mStatusBarBackground != null))
      if (mLastInsets == null)
        break label61;
    label61: for (int i = mLastInsets.getSystemWindowInsetTop(); ; i = 0)
    {
      if (i > 0)
      {
  ...
  protected void onMeasure(int paramInt1, int paramInt2)
  {
    ...
    label512: setMeasuredDimension(ViewCompat.resolveSizeAndState(i2, paramInt1, 0xFF000000 & n), ViewCompat.resolveSizeAndState(i1, paramInt2, n << 16));
  }
  ...
  public void onNestedPreScroll(View paramView, int paramInt1, int paramInt2, int[] paramArrayOfInt)
  {
    ...
    if (k < i2)
    {
      ...
      if (paramInt1 > 0)
      {
        ...
        label146: if (paramInt2 <= 0)
          break label193;
      }
      label193: for (j = Math.max(j, mTempIntPair[1]); ; j = Math.min(j, mTempIntPair[1]))
      {
        m = 1;
        n = i;
        i1 = j;
