...
public class SwipeDismissBehavior<V extends View> extends CoordinatorLayout.Behavior<V>
{
    ...
    public void onViewReleased(View paramAnonymousView, float paramAnonymousFloat1, float paramAnonymousFloat2)
    {
      ...
      if (shouldDismiss(paramAnonymousView, paramAnonymousFloat1))
        if (paramAnonymousView.getLeft() < mOriginalCapturedViewLeft)
        {
          ...
          bool = true;
          label41: if (!mViewDragHelper.settleCapturedViewAt(i, paramAnonymousView.getTop()))
            break label100;
  ...
  private void ensureViewDragHelper(ViewGroup paramViewGroup)
  {
    if (mViewDragHelper == null)
      if (!mSensitivitySet)
        break label33;
    label33: for (paramViewGroup = ViewDragHelper.create(paramViewGroup, mSensitivity, mDragCallback); ; paramViewGroup = ViewDragHelper.create(paramViewGroup, mDragCallback))
    {
